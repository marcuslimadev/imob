    ...
    try {
      const companies = await directusServer.request(
        readItems('companies', {
          ...
          fields: ['id', 'slug', 'storefront_template_id'],
          limit: 1
        })
      );
      ...
      if (companies.length > 0) {
        ...
        // Injetar template ID para vitrines públicas
        if (companies[0].storefront_template_id) {
          response.headers.set('x-storefront-template-id', companies[0].storefront_template_id.toString());
        }
      }
    } catch (error) {
