...
export default (router, { services, logger, database }) => {
	...
	async function processIncomingMessage(webhookData, companySettings, { services, logger, database, req }) {
		...
		try {
			...
			// 4. Se for áudio, transcrever via OpenAI
			let finalBody = body;
			let transcriptionText = null;
			
			if (messageType === 'audio' && mediaUrl) {
				logger.info('🎤 Áudio detectado, transcrevendo via Whisper...');
				...
				try {
					...
					// Chamar endpoint OpenAI Whisper
					const transcribeResponse = await fetch(`${process.env.PUBLIC_URL}/openai/transcribe`, {
						method: 'POST',
						headers: { 'Content-Type': 'application/json' },
						body: JSON.stringify({
							audio_url: mediaUrl,
					...
					if (transcribeResult.success && transcribeResult.text) {
						...
						finalBody = transcriptionText;
						
						// Atualizar mensagem com transcrição
						await mensagensService.updateOne(mensagem.id, {
							content: `[ÁUDIO] ${transcriptionText}`,
							metadata: JSON.stringify({ 
								transcribed: true, 
								original_media: mediaUrl 
							})
						});
						
						logger.info('✅ Áudio transcrito:', transcriptionText.substring(0, 100) + '...');
					} else {
						logger.error('❌ Falha na transcrição:', transcribeResult.error);
						finalBody = '[Áudio - falha na transcrição]';
					}
				} catch (error) {
					logger.error('❌ Erro ao transcrever áudio:', error.message);
					finalBody = '[Áudio - erro na transcrição]';
				}
			}
			...
			if (conversaAtual?.lead_id) {
				...
				if (hasEnoughDataForMatching(leadAtualizado)) {
					...
					// Ainda coletando dados - usar IA para resposta inteligente
					logger.info('📝 Ainda coletando informações, processando com IA...');
					
					try {
						// Buscar imóveis disponíveis para contexto
						const propertiesService = new ItemsService('properties', { schema: req.schema });
						const availableProperties = await propertiesService.readByQuery({
							filter: { 
								company_id: { _eq: companyId },
								status: { _eq: 'published' }
							},
							limit: 10,
							fields: ['codigo_imovel', 'tipo_imovel', 'bairro', 'cidade', 'valor_venda', 'dormitorios', 'suites', 'descricao', 'imagens']
						});

						// Chamar OpenAI para processar mensagem
						const aiResponse = await fetch(`${process.env.PUBLIC_URL}/openai/process-message`, {
							method: 'POST',
							headers: { 'Content-Type': 'application/json' },
							body: JSON.stringify({
								message: finalBody,
								context: `Conversação em andamento. Stage: ${conversaAtual.stage}`,
								isFromAudio: messageType === 'audio',
								availableProperties,
						...
						let collectingResponse;
						if (aiResult.success && aiResult.response) {
							collectingResponse = aiResult.response;
							logger.info('✅ Resposta gerada pela IA:', collectingResponse.substring(0, 100) + '...');
							
							// Se IA extraiu dados, atualizar lead
							if (aiResult.extracted_data && Object.keys(aiResult.extracted_data).length > 0) {
								await leadsService.updateOne(conversaAtual.lead_id, {
									...aiResult.extracted_data,
									ultima_interacao: new Date()
								});
								logger.info('✅ Dados extraídos pela IA e salvos no lead');
							}
						} else {
							// Fallback se IA falhar
							collectingResponse = `Entendi! Pode me contar mais sobre o que você procura? Por exemplo:\n\n• Qual o seu orçamento?\n• Prefere alguma região específica?\n• Quantos quartos precisa?`;
							logger.warn('⚠️ IA não gerou resposta, usando fallback');
						}
						...
						const fallbackMsg = `Entendi! Pode me contar mais sobre o que você procura?`;
